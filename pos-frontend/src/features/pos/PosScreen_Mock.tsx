  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-100">
          ...
          {categories.map((c) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-sm ${
                activeCat === c.id ? "bg-indigo-600" : "bg-slate-800 hover:bg-slate-700"
              }`}
            >
              {c.name}
            </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {products.map((p) => (
            <button
              key={p.id}
              onClick={() => addToCart(p)}
              className="rounded-xl bg-slate-800 p-3 text-left hover:bg-slate-700"
            >
              ...
              <div className="text-sm text-slate-400">${p.price.toFixed(2)}</div>
            </button>
          ))}
          {products.length === 0 && (
      ...
      <div className="w-[420px] flex flex-col">
        ...
        {/* Barcode box */}
        <form onSubmit={handleBarcodeEnter} className="p-4 border-b border-slate-800 flex items-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {cart.map((l) => (
            <div key={l.product.id} className="flex items-center justify-between bg-slate-800 rounded-lg p-3">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => changeQty(l.product.id, -1)} className="rounded bg-slate-700 p-1 hover:bg-slate-600">
        ...
        <div className="border-t border-slate-800 p-4 space-y-2">
          ...
          {message && (
            <div className="mt-3 rounded-lg bg-slate-800 p-2 text-sm text-slate-200">{message}</div>
          )}
        </div>
      </div>
    </div>
