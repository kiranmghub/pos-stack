...
export default function StartReturnModal({ open, onClose, sale, onSubmit }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[70]">
      ...
      <div className="absolute left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
        {/* Header */}
        <div className="border-b border-border px-5 py-3">
          <div className="text-base font-semibold text-foreground">Start return</div>
          <div className="mt-1 text-xs text-muted-foreground">
            Sale <span className="text-foreground">{sale.receipt_no || `#${sale.id}`}</span>
          </div>
        </div>
        ...
        <div className="space-y-4 px-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-3 rounded-lg border border-border p-3 text-sm">
            <div><span className="text-muted-foreground">Store:</span> {sale.store_name || "—"}</div>
            <div><span className="text-muted-foreground">Cashier:</span> {sale.cashier_name || "—"}</div>
            <div><span className="text-muted-foreground">Date:</span> {sale.created_at ? new Date(sale.created_at).toLocaleString() : "—"}</div>
            <div><span className="text-muted-foreground">Total:</span> {String(sale.total ?? "—")}</div>
          </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-border px-5 py-3">
          <button
            className="rounded-md px-3 py-1.5 text-sm text-muted-foreground hover:bg-white/5"
            onClick={onClose}
            disabled={submitting}
          >
            Cancel
