    ...
    <div className="rounded-2xl border border-cyan-500/20 bg-background/80">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/5 text-left text-[11px] uppercase tracking-[0.2em] text-cyan-200/70">
          ...
          <tbody className="divide-y divide-white/5">
            {loading && (
              <tr>
                <td colSpan={5} className="px-4 py-4 text-center text-cyan-200/60">
            ...
            {!loading &&
              rows.map((row) => (
                <tr key={row.id} className="hover:bg-cyan-500/5">
                  <td className="px-4 py-3">
                    <div className="text-sm text-white">{row.receipt_no || `Sale #${row.id}`}</div>
                    <div className="text-xs text-cyan-200/70">{new Date(row.created_at).toLocaleString()}</div>
                  </td>
                  <td className="px-4 py-3 text-sm text-white">{row.store_name || "—"}</td>
                  <td className="px-4 py-3 text-sm text-white">{row.cashier_name || "—"}</td>
                  <td className="px-4 py-3 text-right tabular-nums text-cyan-100">{row.tax_total}</td>
      ...
      <div className="flex flex-col gap-2 border-t border-white/5 px-4 py-3 text-xs text-cyan-200/70 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="rounded-md border border-cyan-500/40 px-2 py-1 disabled:opacity-40"
